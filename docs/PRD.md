...
### 2026-05
- **Mobile-first PWA overhaul** — major batch of changes to make the app feel native on phone.
  * **PWA primitives**: `app/manifest.ts` with name + theme + icons. Programmatic icons via Next 16's `app/icon.tsx` / `icon1.tsx` / `icon2.tsx` / `apple-icon.tsx` (32 / 192 / 512 / 180). Service worker at `public/sw.js` registered in production via `ServiceWorkerRegistration.tsx`. Viewport meta with `viewportFit: "cover"` for safe areas. Trainers can Add to Home Screen on iOS/Android and the app opens standalone.
  * **Bottom navigation** on screens < md: Home / Clients / Chats / Inbox / More. Chats shows unread badge, Inbox shows escalation count (live). "More" opens a slide-up bottom sheet with Plans / Payments / Settings. Active tab gets a teal underscore + filled icon. 56px tall, respects `env(safe-area-inset-bottom)`.
  * **Top header hidden on mobile** (`hidden md:flex`) — search, WhatsApp pill, profile move into the More sheet or their natural pages. Mobile sidebar slide-in dropped (bottom nav replaces it).
  * **Dashboard**: metric cards stack one-per-row on mobile (was 2-per-row). Action Needed escalation buttons bumped to min-h-[40px]. Today's check-in row buttons bumped to min-h-[36px] with active states.
  * **Clients**: table swapped for a card list on mobile — each card shows avatar + name + plan + status pill + compliance bar + weight. Card is fully tappable.
  * **Conversations**: composer redesigned with bigger 44px tap targets. The right-side primary button is "Send" when there's a draft, switches to a prominent voice-note mic when empty (matches the highest-frequency mobile action). Image and camera buttons use real `<input type="file" accept="image/*,video/*" capture>` so they open native pickers/cameras on mobile. Container height uses `100dvh` minus safe areas + bottom nav.
  * **Inbox**: "Use this" buttons under suggested replies bumped to min-h-[40px], full-width on mobile. Open conversation / Tap to call / Send voice note bumped to min-h-[44px] (iOS HIG).
  * **Plan builder**: sticky bottom action bar (Save plan + Send plan) on mobile sits above the bottom nav. Exercise library swapped for a slide-up drawer on mobile — opened via a floating Dumbbell FAB on the Workouts tab. Picking an exercise auto-dismisses the drawer.
