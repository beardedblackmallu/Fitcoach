...
| Splash screen not pixel-identical on Android vs iOS | Splash | Low | Android uses a vector drawable path; iOS uses a UILabel (Futura Bold). They look similar but aren't identical. Fix: export a PNG from Figma and add to both android/res/drawable/ and ios/App/App/Assets.xcassets/Splash.imageset/. 30-min design task. |
| iOS native build requires full Xcode | Capacitor / iOS | Med | Local dev env has only Command Line Tools — `pod install` fails because `xcodebuild` is missing. User action: install Xcode from App Store (7+ GB). Until then, iOS smoke test (CP4 test 6, CP5 test 6) is blocked. Android build is unaffected. |
