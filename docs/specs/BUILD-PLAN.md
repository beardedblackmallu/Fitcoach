...
- **Razorpay webhook** — must be registered in Razorpay Dashboard → Settings → Webhooks with URL `https://gcendipptiwpipadqzan.supabase.co/functions/v1/razorpay-webhook`, secret `[stored in .env.local — see RAZORPAY_WEBHOOK_SECRET]`, event `payment.captured`.
